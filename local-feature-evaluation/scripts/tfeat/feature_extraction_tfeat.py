...
import torch
from models import tfeat_model
...
def parse_args():
    ...
    parser.add_argument("--model_path", required=True,
                        help="Path to the pretained tfeat model")
    ...
    for i, image_name in enumerate(image_names):
        ...
        with torch.no_grad():
            for i in range(0, patches.shape[0], args.batch_size):
                ...
                with torch.no_grad():
                    descriptors_batch = tfeat(patches_batch)
                descriptors.append(descriptors_batch.detach().cpu().numpy().reshape(-1, 128))
