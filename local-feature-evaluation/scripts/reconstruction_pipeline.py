...
def import_matches(args):
    ...
    for image_name, image_id in images.items():
        ...
        # offset is consistent with that of the line 62 in colmap_export.py.
        # CAUTION: please comment this line for learned keypoints.
        keypoints[:, :2] += 0.5
...
def reconstruct(args):
    ...
    time_mapper = end_mapper - start_mapper
    ...
    for model in models:
        subprocess.call([os.path.join(args.colmap_path, "colmap"),
                         ...
                         "--output_type", "TXT"])
        with open(os.path.join(sparse_path, model, "images.txt"), "r") as fid:
            for line in fid:
                if line.startswith("# Number of images"):
                    # num_images = int(line.split()[-1])
                    num_images = int(line.split(',')[0].split()[-1])
...
def main():
    ...
    print("| " + " | ".join(
            map(str, [os.path.basename(args.dataset_path),
                      ...
                      reconstruction_stats["time_mapper"],
                      "",
                      "",
                      "",
                      "",
    ...
    with open(os.path.join(args.dataset_path, "statistics.txt"), "w") as fid:
        fid.write(str(matching_stats))
        fid.write("\n")
        fid.write(str(reconstruction_stats))
